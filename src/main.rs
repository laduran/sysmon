...
fn main() {
    ...
    app.connect_activate(|app| {
        ...
        let (main_box, widgets) = create_ui();
        ...
        glib::timeout_add_local(Duration::from_secs(1), move || {
            ...
            w.cpu_progress.set_fraction(cpu_frac);

            // ── Memory ──────────────────────────────────────────────────────
            let to_gb = |b: u64| b as f64 / (1024.0 * 1024.0 * 1024.0);
            let to_mb = |b: u64| b as f64 / (1024.0 * 1024.0);
            w.mem_total.set_text(&format!("Total: {:.2} GB", to_gb(mem_stats.total)));
            w.mem_used.set_text( &format!("Used:  {:.2} GB", to_gb(mem_stats.used)));
            w.mem_free.set_text( &format!("Free:  {:.2} GB", to_gb(mem_stats.free)));
            w.mem_swap.set_text( &format!("Swap:  {:.0} MB used / {:.0} MB total",
                                          to_mb(mem_stats.swap_used),
                                          to_mb(mem_stats.swap_total)));
            if mem_stats.total > 0 {
                w.mem_progress.set_fraction(
                    (mem_stats.used as f64 / mem_stats.total as f64).clamp(0.0, 1.0)
                );
            }
